...
- Engagement sentence: add this exact sentence near the ending only, not near the opening: `如果觉得说得有道理，可以关注、点赞、评论、推荐。` Verify `contains_opening_engagement: false` after `draft/get`.
...
6. Verify with `draft/get`, decoding `response.content` as UTF-8.
   - If `draft/add` returns a `media_id` but `draft/get` fails transiently (for example TLS/SSL EOF or network timeout), do not assume the draft failed and do not create a duplicate immediately. Use `draft/batchget` or retry `draft/get` to find/verify the just-created title/media_id, then update the result JSON as `draft_created`.
   - Sanitize logs/results before saving or reporting: never leave `access_token=...` URLs inside `failure_reason` or summary JSON.
